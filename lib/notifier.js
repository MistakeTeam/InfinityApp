...
function notify(title, data) {
    ...
    if (!options.noNotify) {
        ...
        <div class="notification-push animation-default" index="${notifyIndex}" style="left: calc(100% - 300px); opacity: 0;">
            <img src="${notifications[notifyIndex].icon}" style="max-height: 32px;"></img>
            <div style="flex-direction: column; display: flex; padding-left: 5px;">
...
eventEmitter.on('statusbar-item-notification', () => {
    if ($('.central-notifications').length != 0) {
        $('.central-notifications').css('left', 'calc(100% - 300px)');
        $('.central-notifications').css('opacity', '0');
        $('.central-notifications').css('z-index', '0');
    ...
    setTimeout(() => {
        ...
        notifications.forEach((notification, index) => {
            ...
            <div class="${classNames}" index="${index}">
                <img src="${notification.icon}" style="max-height: 32px;"></img>
                <div style="flex-direction: column; display: flex; padding-left: 5px;">
