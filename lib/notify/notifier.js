...
class Notifier {
    ...
    notify(title, data) {
        ...
        if (options.vertical && options.buttons && options.buttons.length) {
            verticalSpace = Math.min(options.buttons.length * 20, 85);
        } else {
        ...
        let windowOptions = {
            width: 440,
            height: this.notificationHeight + verticalSpace + this.stackGap,
            /* stack gap now supported, though not sure why the current gap would not be sufficient */
            x: size.width - 440,
            y: 0,
            /* this is because we set the poper position when we pop it from the queue */
            frame: false,
            transparent: true,
            resizable: false,
            alwaysOnTop: false,
